...
2. **WP02-C Second-fixture cross-language validation (COMPLETE on `feat/civic-wp02c-projection-second-fixture` branch, awaiting PR)**: A 7th gate is implemented as a separate `FlatbuffersSecondFixtureCheck.cs` that cross-validates the Python reader on `sample-projection.bin` (the ProjectionDelta union arm of `RootPayload`). The new fixture is generated deterministically by `.agileplus/civic-warfare-program/contracts/fixtures/generate-projection-fixture.sh` from a hand-crafted JSON sample and committed alongside the script. The Python reader is extended to decode ProjectionDelta. A dependency-free drift runner `tests/run_wp02c_drift.py` covers 8 mutation cases (empty, wrong file_identifier, truncated root uoffset, truncated file_identifier, truncated root table, corrupted vtable_size, corrupted decisions_count, corrupted payload_type discriminator). The C# side keeps the existing WP02-B reader untouched — the 7th gate invokes Python as a subprocess, mirroring the WP02-B cross-language check pattern. Python reader output exactly matches `flatc --json --raw-binary --strict-json` canonical decode for both fixtures. Files: `tests/flatbuffers_reader.py` (Python reader, ~440 lines), `tests/run_wp02c_drift.py` (8-case drift runner), `CivicSurvival.PublicAudit/FlatbuffersSecondFixtureCheck.cs` (subprocess invocation + JSON parse + field validation), `.agileplus/civic-warfare-program/contracts/fixtures/sample-projection.bin` (golden fixture, ~384 bytes, generated by `flatc --binary --strict-json --force-defaults`), `.agileplus/civic-warfare-program/contracts/fixtures/generate-projection-fixture.sh` (reproducible generator).

3. **Wire the C# 7th gate into `Program.cs`** (not yet done; `FlatbuffersSecondFixtureCheck.cs` exists and compiles but the orchestrator currently only invokes the WP02-B gate). When wired, the 7th gate field `flatbuffersSecondFixture` will appear in the JSON output and `test_runner.sh` will assert against it.

4. **WP02-D Rust encoder (future)**: Add a Rust crate under `native/` that uses the canonical `flatbuffers` crate to **encode** a `ProjectionDelta` and round-trip it back to JSON via `flatc`. The hand-written C# and Python readers should both decode the Rust-produced binary identically. Closes the loop on language-agnostic wire contracts.
